$(document).ready(function(){
  ...
  function twTriTableau() {
    ...
    for(i = 0; i < nNbrLigne; i++){
      if(aColonne[i][6]){
        var last_col = aColonne[i][6];
        aColonne[i].pop();
      }
      if(aColonne[i][4] && !aColonne[i][5]){
        var last_col4 = aColonne[i][4];
        aColonne[i].pop();
      }

      if(aColonne[i][4]){
        var date_pret = aColonne[i][4];
        date_pret = date_pret.slice(1, 11);
      }

      var today = new Date().toJSON().slice(0,10);
      today = String(today);

      if(date_pret && date_pret == today){
        aColonne[i] = "<td class='color' style='color: red'>"+aColonne[i].join("</td><td class='color' style='color: red'>")+"</td>";
      } else{
        aColonne[i] = "<td class='color'>"+aColonne[i].join("</td><td class='color'>")+"</td>";
      }

      if(last_col4)
        aColonne[i] += "<td class='td_action'>"+ last_col4 +"</td>";
      if(last_col){
        aColonne[i] += "<td class='td_action'>"+ last_col +"</td>";
      }
